...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embed = tf.get_variable('embedding_tensor', dtype=tf.float32, initializer=self.pretrained_embeddings)
        embed_lookup = tf.nn.embedding_lookup(embed, self.input_placeholder)
        embeddings = tf.reshape(embed_lookup, shape=(-1, self.max_length, self.config.n_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        state = tf.zeros((tf.shape(self.input_placeholder)[0], self.config.hidden_size), dtype=tf.float32)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables() #reuse mode
                output, state = cell(x[:, time_step, :], state)
                output_drop = tf.nn.dropout(output, 1 - dropout_rate)
                pred = tf.matmul(output_drop, U) + b2
                preds.append(pred)
                ### END YOUR CODE

        # Make sure to reshape @preds here.
        ### YOUR CODE HERE (~2-4 lines)
        preds = tf.stack(preds, axis=1) #(None, T, C)
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        ce_unmask = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
        ce_mask = tf.boolean_mask(ce_unmask, self.mask_placeholder)
        loss = tf.reduce_mean(ce_mask)
        ### END YOUR CODE
        return loss
