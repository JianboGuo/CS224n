...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self):
        ...
        _, state = tf.nn.dynamic_rnn(cell, inputs=self.inputs_placeholder, initial_state=tf.zeros((tf.shape(self.inputs_placeholder)[0], cell.state_size)))
    ...
    def add_training_op(self, loss):
        ...
        gradients_ = [gv[0] for gv in gradients_vars]
        vars_ = [gv[1] for gv in gradients_vars]
        self.grad_norm = tf.global_norm(gradients_)
        if self.config.clip_gradients:
            gradients_clip_, _ = tf.clip_by_global_norm(gradients_, self.config.max_grad_norm, use_norm = self.grad_norm)
            gradients_vars = zip(gradients_clip_, vars_)

        train_op = optimizer.apply_gradients(gradients_vars)
