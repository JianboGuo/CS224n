...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            # b_r = tf.get_variable('b_r', shape=(self.state_size), dtype=tf.float32, initializer=tf.constant_initializer(0))
            # b_z = tf.get_variable('b_z', shape=(self.state_size), dtype=tf.float32, initializer=tf.constant_initializer(0))
            # b_o = tf.get_variable('b_o', shape=(self.state_size), dtype=tf.float32, initializer=tf.constant_initializer(0))
