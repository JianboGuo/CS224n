...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embeddings_vacab = tf.Variable(self.pretrained_embeddings, dtype=tf.float32)
        embeddings = tf.reshape(tf.nn.embedding_lookup(embeddings_vacab, self.input_placeholder), (-1, self.config.n_window_features * self.config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable('b1', initializer=tf.zeros((1, self.config.hidden_size)))
        h = tf.nn.relu(tf.matmul(x, W) + b1)
        h_drop = tf.nn.dropout(h, 1 - dropout_rate)

        U = tf.get_variable('U', shape=(self.config.hidden_size, self.config.n_classes),
                            dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable('b2', initializer=tf.zeros((1, self.config.n_classes)))
    ...
    def add_loss_op(self, pred):
        ...
        #nn.sparse_softmax_cross_entropy_with_logits: single specific index; softmax_cross_entropy_with_logits: one-hot
