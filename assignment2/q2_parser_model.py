...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.dropout_placeholder = tf.placeholder(dtype=tf.float32, shape=()) #shape=() for a scalar
        ### END YOUR CODE
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        self.embeddings = tf.Variable(self.pretrained_embeddings, dtype=tf.float32)
        embeddings = tf.reshape(tf.nn.embedding_lookup(self.embeddings, self.input_placeholder), (-1, self.config.n_features * self.config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        self.W = tf.get_variable('hidden_layer_w', dtype=tf.float32, 
                                initializer=xavier_initializer((self.config.n_features*self.config.embed_size , self.config.hidden_size)))
        self.b1 = tf.get_variable('hidden_layer_b', initializer=tf.zeros((1, self.config.hidden_size)))
        self.h = tf.nn.relu(tf.matmul(x, self.W) + self.b1)
        self.h_drop = tf.nn.dropout(self.h, 1 - self.dropout_placeholder)
        self.U = tf.get_variable('output_layer_w', dtype=tf.float32,
                                initializer=xavier_initializer((self.config.hidden_size, self.config.n_classes)))
        self.b2 = tf.get_variable('output_layer_b', initializer=tf.zeros((1, self.config.n_classes)))
